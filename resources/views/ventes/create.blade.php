...
<div class="topbar">
  ...
  <a href="{{ route('notifications') }}" class="position-relative me-4" style="display:inline-block;">
                      <i class="bi bi-bell" style="font-size:26px; position: relative;color:#fffff"></i>
                      @if(isset($notifications) && $notifications->count() > 0)
                          <span class="notification-count animate-bounce">
                              {{ $notifications->count() }}
                           </span>
                      @endif
                  </a>
    ...
    <button id="clear-cart-btn" class="btn btn-sm btn-outline-light" title="Vider le panier"><i class="bi bi-trash"></i></button>
  </div>
</div>
      ...
      <div class="col-12 col-lg-8">
        ...
        <div class="card mb-3">
          <div class="card-body">
            <h5 class="section-title">
              ...
              <span class="badge bg-success ms-2" id="items-count">0</span>
            </h5>
            ...
            <div class="total-display bg-light rounded p-3">
              <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <span id="total-vente" class="fw-bold">0 FCFA</span>
              </div>
              <div class="d-flex justify-content-between align-items-center">
                <span class="fw-semibold">Montant à payer:</span>
                <span id="montant-a-payer" class="fw-bold fs-5">0 FCFA</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card left-panel-sticky border-0">
          <div class="card-body">
            ...
            <form id="vente-form" method="POST" action="{{ route('ventes.store') }}">
              @csrf
              <div class="row g-3">
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                  ...
                  <input id="nom_client" name="nom_client" type="text" class="form-control" placeholder="Nom client">
                </div>
                <div class="col-md-4">
                  <label for="contact_client" class="form-label fw-semibold">Contact</label>
                  <input id="contact_client" name="contact_client" type="text" class="form-control" placeholder="Téléphone ou email">
                </div>
              </div>
              ...
              <div class="row g-3 align-items-end">
                ...
                <div class="col-md-6">
                  ...
                  <div class="input-group">
                    ...
                    <input id="montant_regle" name="montant_regle" type="number" step="0.01" class="form-control" value="0" required>
                  </div>
              ...
              <div class="mt-3">
                ...
                <textarea id="remarques" name="remarques" rows="2" class="form-control" placeholder="Notes supplémentaires..."></textarea>
              </div>

              <input type="hidden" name="lignes" id="lignes-input" value="[]">

              <div class="d-flex gap-2 mt-4">
                <a href="{{ route('ventes.index') }}" class="btn btn-outline-secondary flex-fill">
      ...
      <div class="col-12 col-lg-4">
        <div class="card border-0 " style="background-color: #000000; color: #fff;">
          <div class="card-body">
            ...
            <div class="mb-2 small-muted text-white">Articles: <span id="items-count-2">0</span></div>
            <div class="fw-semibold fs-4 text-white" id="total-vente-side">0 FCFA</div>
...
/* ===========================
   ...
   ===========================
   Remplace @json($produits) par tes produits injectés côté serveur.
*/
let allProducts = @json($produits);

// État local
let productsState = allProducts.map(p => ({ ...p })); // copie pour gestion visuelle du stock
let produitsVente = []; // lignes du panier {id, produit_id, produit_text, quantite, prix_unitaire, sous_total}
let nextId = 1;
...
btnAjouterProduit.addEventListener('click', () => {
  ...
  calculateTotal();
  productModal.hide();
  flashToast('Produit ajouté au panier','success');
});
...
function updateCartDisplay() {
  if (!produitsVente.length) {
    ...
    produitsListTbody.innerHTML = produitsVente.map(l => `
      <tr data-local-id="${l.id}">
        <td class="align-middle">${escapeHtml(l.produit_text)}</td>
        <td class="align-middle">
          <div class="quantity-controls">
            <button class="quantity-btn" type="button" onclick="changeQty(${l.id}, -1)">−</button>
            <span class="form-control text-center" style="max-width:60px" type="number" value="${l.quantite}" onchange="setQty(${l.id}, this.value)" min="1">${l.quantite}</span>
            <button class="quantity-btn" type="button" onclick="changeQty(${l.id}, 1)">+</button>
          </div>
        </td>
        <td class="align-middle">${formatPrice(l.prix_unitaire)} FCFA</td>
        <td class="align-middle fw-semibold">${formatPrice(l.sous_total)} FCFA</td>
        <td class="align-middle">
          <button class="btn btn-sm btn-outline-danger" type="button" onclick="removeLine(${l.id})" title="Supprimer">
...
function changeQty(localId, delta) {
  const line = produitsVente.find(l => l.id === localId); 
  if (!line) return;
  
  const prod = productsState.find(p => p.id === line.produit_id);
  const available = (prod ? prod.quantite_stock : 0) + line.quantite;
  ...
  if (prod) prod.quantite_stock = Math.max(0, available - newQty);
  
  updateCartDisplay();
  calculateTotal();
}

function setQty(localId, value) {
  ...
  if (prod) prod.quantite_stock = Math.max(0, available - v);
...
function removeLine(localId) {
  ...
  produitsVente = produitsVente.filter(l => l.id !== localId);
  updateCartDisplay(); 
  calculateTotal();
...
clearCartBtn.addEventListener('click', () => {
  if (!produitsVente.length) { 
    flashToast('Le panier est déjà vide','info'); 
    return; 
  }
  
  if (!confirm('Vider le panier ?')) return;
  
  produitsVente.forEach(l => {
    const p = productsState.find(x => x.id === l.produit_id); 
    if (p) p.quantite_stock += l.quantite;
  });
  
  produitsVente = []; 
  updateCartDisplay(); 
  calculateTotal();
  flashToast('Panier vidé', 'success');
});
...
function calculateTotal() {
  ...
  submitBtn.disabled = produitsVente.length === 0;
}
...
document.getElementById('vente-form').addEventListener('submit', function(ev) {
  ...
  if (paid < total) {
    if (!confirm(`Le montant réglé (${formatPrice(paid)} FCFA) est inférieur au total (${formatPrice(total)} FCFA). Voulez-vous continuer ?`)) {
      ev.preventDefault(); 
      return;
    }
  }
...
window.supprimerProduit = removeLine; // alias for older code compatibility
window.selectProduct = (id) => openProductModal(id); // alias
